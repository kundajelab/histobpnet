...
def get_gc_matched_negatives(
    ...
    n_path = os.path.join(output_prefix, "_auxiliary/negatives.bed")
    ...
    plt.savefig(os.path.join(output_prefix, "_auxiliary/negatives_compared_with_foreground.png"))

    return n_path

def get_gc_content(genome, peaks_bed: str, chrom_sizes, inputlen: int, output_prefix: str):
    """
    ...
    print("Number of peaks in the input narrowPeak file: " + str(data.shape[0]))
    ...
    print("Number of regions filtered: " + str(filtered_peaks))
    pct_filtered = round(filtered_peaks*100.0/data.shape[0], 3)
    print(f"Percentage of regions filtered: {str(pct_filtered)}%" )
    if pct_filtered > 25:
    	print("WARNING: Percentage of regions filtered is high (>25%), your genome is likely very small - consider using a smaller inputlen")
...
def get_parsers():
    ...
    # parser.add_argument("-gwgc","--genomewide_gc", required=True, help="genomewide gc bed file")
...
# TODO fix output_prefix stuff... and use os path join
def main(instance_id: str):
    args_d, output_dir, logger = setup(instance_id)

    os.makedirs(output_dir + "_auxiliary/", exist_ok=False)

    # get gc content in peaks
    get_gc_content(args_d["genome"], args_d["peaks"], args_d["chrom_sizes"], args_d["inputlen"], args_d["output_prefix"] + "_auxiliary/foreground.gc")
    ...
    def get_excluded_regions(region_type: str):
        ...
        os.system("bedtools slop -i {peaks} -g {chrom_sizes} -b {flank_size} > {output}".format(
            ...
            output=args_d["output_prefix"] + "_auxiliary/{region_type}_slop.bed")
        )
        exclude_bed = pl.read_csv(args_d["output_prefix"] + "_auxiliary/{region_type}_slop.bed", separator="\t", has_header=False, columns=[0,1,2])
        return exclude_bed
    ...
    exclude_bed.to_csv(args_d["output_prefix"] + "_auxiliary/exclude_unmerged.bed", sep="\t", header=False, index=False)

    # merge the exclude_bed regions
    os.system("bedtools sort -i {inputb} | bedtools merge -i stdin > {output}".format(
        inputb=args_d["output_prefix"] + "_auxiliary/exclude_unmerged.bed",
        output=args_d["output_prefix"] + "_auxiliary/exclude.bed")
    )
    ...
    bedtools_command = "bedtools intersect -v -a {genomewide_gc} -b {exclude_bed}  > {candidate_bed}".format(
        ...
        exclude_bed=args_d["output_prefix"]+"_auxiliary/exclude.bed",
        candidate_bed=args_d["output_prefix"]+"_auxiliary/candidates.bed"
    )
    ...
    n_path = get_gc_matched_negatives(
        output_dir,
        candidate_negatives=args_d["output_prefix"]+"_auxiliary/candidates.bed",
        foreground_gc_bed=args_d["output_prefix"]+"_auxiliary/foreground.gc.bed",
        output_prefix=args_d["output_prefix"]+"_auxiliary/negatives",
    ...
    negatives.write_csv(
        args_d["output_prefix"] + "_negatives.bed",
        separator="\t",
        include_header=False,
        include_index=False
    )

    # TODO convert all prints to logger.add to log
